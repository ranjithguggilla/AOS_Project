...
export const listProducts =  (keyword = '') => async(dispatch) => {
    try {
        ...
        const { data }  = keyword ? await axios.get(`/api/products/search?keyword=${keyword}`) : await axios.get(`/api/products`)
...
export const ListproductbyCg = (Cg) => async (dispatch) =>{
    try {
        dispatch({type : PRODUCT_LIST_REQUEST })
        const {data} = await axios.get(`/api/products?category=${Cg}`)
